# ----------------------------------PROJECT:- ONLINE GROCERY RECOMMENDATION--------------------------------------------
# A PROJECT BY ANANYA ROY
...
def time():  # function to display date and time
    ...
    for i in dateV:
        ...
        DT += '/'
    DT = DT[:len(DT) - 1]
    date = tk.Label(root, bg='midnight blue', fg='red', text='Date:  ' + DT + '\t\tTime:  ' + str(q), bd=3)
...
def addItem():  # add extra item to the bill
    ...
    productE = tk.Label(root, bg='midnight blue', text=str(pro), relief='flat', width=50)
    productE.grid(column=40, row=row)
    priceE = tk.Label(root, bg='midnight blue', text=str(pri), relief='flat')
    priceE.grid(column=80, row=row)
    quantityE = tk.Label(root, bg='midnight blue', text=str(qua), relief='flat')
    quantityE.grid(column=120, row=row)
    productE.destroy()
    priceE.destroy()
    quantityE.destroy()

    SNo = tk.Label(root, bg='midnight blue', fg='red', text=str(num))
    ...
    quantityE.grid(column=120, row=row)
    amountE = tk.Label(root, bg='cyan', text=str(amo), relief='flat')
    amountE.grid(column=200, row=row - 1)
    Total = tk.Label(root, bg='midnight blue', text="Total Amount:" + str(sum(totalAmount)), fg='Red')
    ...
    class SampleApp(tk.Tk):
        ...
        def OnDouble(self, event):
            ...
            try:
                ...
                Rec_pric = (GP.iloc[[list(GP['Product']).index(value[0])], :].values[0][1] +
                            GP.iloc[[list(GP['Product']).index(value[1])], :].values[0][1]) - (0.1 * (
                            GP.iloc[[list(GP['Product']).index(value[0])], :].values[0][1] +
                            GP.iloc[[list(GP['Product']).index(value[1])], :].values[0][1]))
...
# title
heading = tk.Label(root, text='Products billing\n', font=('Tahoma', 30), fg='Yellow', bg='midnight blue', bd=3)
heading.grid(column=70, row=1)

# customer Details
nameL = tk.Label(root, bg='midnight blue', fg='red', text='Name')
nameL.grid(column=2, row=4)
nameE = AutocompleteEntry(lista, root)
nameE.grid(column=40, row=4)
NumberL = tk.Label(root, bg='midnight blue', fg='red', text='Mobile Number  ')
NumberL.grid(column=75, row=4)
...
SNO = tk.Label(root, bg='midnight blue', fg='red', text='SNo.')
SNO.grid(column=40, row=7)
product = tk.Label(root, bg='midnight blue', fg='red', text='Product')
product.grid(column=40, row=7)
price = tk.Label(root, bg='midnight blue', fg='red', text='Price')
price.grid(column=80, row=7)
quantity = tk.Label(root, bg='midnight blue', fg='red', text='Quantity')
quantity.grid(column=120, row=7)
amount = tk.Label(root, bg='midnight blue', fg='red', text='Amount')
...
amountE = tk.Label(root, bg='midnight blue', relief='flat')
...
PrintButton = tk.Button(root, text='Print Bill', font=('Tahoma'), width=20, command=print_bill, bg='green2', fg='Black',
                        bd=5)
PrintButton.grid(column=200, row=34)

SubmitButton = tk.Button(root, text='Recommend', font=('Tahoma'), width=20, command=recommend, bg='light cyan',
                         fg='Black', bd=5)
...
root1.configure(background='midnight blue')
root1.mainloop()
